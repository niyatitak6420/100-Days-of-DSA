    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            if (strcmp(names[i], names[j]) == 0) {
                count++;
                printf("Found another vote for %s (count = %d)\n", names[i], count);
            }
        }
        ...
        if (count > maxCount) {
            ...
            printf("New winner: %s with %d votes\n", winner, maxCount);
        } 
        else if (count == maxCount) {
            if (strcmp(names[i], winner) < 0) {
                strcpy(winner, names[i]);
                printf("Tie detected, lexicographically smaller winner: %s\n", winner);
            }
        }
    }

    printf("\nFinal Winner: %s with %d votes\n", winner, maxCount);
